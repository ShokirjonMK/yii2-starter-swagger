...
class AppController extends Controller
{
    ...
    {
        $actions = self::getControllerActionsFromAliases();

        $auth = Yii::$app->authManager;
        foreach ($actions as $module => $controllers) {
            foreach ($controllers as $controller => $actionList) {
                foreach ($actionList as $action) {
                    ...
                    if (!$auth->getPermission($permissionName)) {
                        ...
                        $permission->description = 'Permission for ' . $permissionName;
                        $auth->add($permission);
                    }
                }
            }
    ...
    public static function getControllerActionsFromAliases()
    {
        // Aliaslar va ularga tegishli yo'llarni ko'rsatamiz
        $aliases = [
        ...
        $controllerNamespaceBase = [
            '@api' => 'api\modules\v1\controllers',
        ...
        foreach ($aliases as $aliasKey => $aliasPath) {
            if (is_dir($aliasPath)) {
                $controllerNamespace = isset($controllerNamespaceBase[$aliasKey]) ? $controllerNamespaceBase[$aliasKey] : 'app\controllers';
                $controllerPath = $aliasPath . '/controllers';

                if (is_dir($controllerPath)) {
                    $controllers = FileHelper::findFiles($controllerPath, ['only' => ['*Controller.php']]);

                    foreach ($controllers as $controller) {
                        $controllerClass = $controllerNamespace . '\\' . basename($controller, '.php');
                        if (class_exists($controllerClass)) {
                            $reflection = new ReflectionClass($controllerClass);
                            if ($reflection->isSubclassOf('yii\web\Controller')) {
                                $controllerName = basename($controller, 'Controller.php');
                                $controllerId = strtolower($controllerName);
                                $methods = $reflection->getMethods(\ReflectionMethod::IS_PUBLIC);

                                foreach ($methods as $method) {
                                    if (strpos($method->name, 'action') === 0 && $method->name !== 'actions') {
                                        $actionId = strtolower(str_replace('action', '', $method->name));
                                        $allActions[$aliasKey][$controllerId][] = $actionId;
                                    }
                                }
                            }
                        }
